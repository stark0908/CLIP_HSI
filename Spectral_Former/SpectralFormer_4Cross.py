...
PROCESSED_ROOT = "/home/23dcs505/datasets/HSI"
...
def get_dataloaders(
    ...
    """
    proc_dir = os.path.join(PROCESSED_ROOT, f"pca{pca_components}_train{split}", dataset_abbr)
...
def main():
    ...
    csv_path = os.path.join(RESULTS_DIR, f"spectralformer_{DATASET_ABBR}_all_splits.csv")
    with open(csv_path, mode='w', newline='') as f:
        writer = csv.writer(f)
        header = ["Split", "OA", "AA", "Kappa"] + CLASS_NAMES
        writer.writerow(header)

    all_results = {}

    for split in splits:
        MODEL_TAG = f"spectralformer_{DATASET_ABBR}_bp{BAND_PATCHES}_split{split}"

        print("=" * 62)
        print(f"  SpectralFormer — Patch-Wise CAF Mode | Split: {split}")
        ...
        # ── Step 1: Load data ─────────────────────────────────────────────────
        print(f"\n[1/4] Loading DataLoaders for Split {split} …")
        ...
        # ── Step 2: Build model ───────────────────────────────────────────────
        print(f"\n[2/4] Building SpectralFormer for Split {split} …")
        model = build_spectralformer()

        # ── Step 3: Train ─────────────────────────────────────────────────────
        print(f"\n[3/4] Training Split {split} …")
        history = train_model(
            model=model,
        ...
        print(f"\n[4/4] Evaluating best model for Split {split} …")
        best_path = os.path.join(MODEL_DIR, f"{MODEL_TAG}_best.pth")
        ...
        with open(csv_path, mode='a', newline='') as f:
            writer = csv.writer(f)
            row = [split, results['OA'], results['AA'], results['Kappa']] + per_class_acc
            writer.writerow(row)
        ...
        print(f"\n{'═' * 62}")
        print(f"  IEEE TGRS Metrics  —  SpectralFormer (Indian Pines) - Split {split}")
        print(f"{'═' * 62}")
        ...
        # Clean up memory before next split
        del model
        del best_model
        del train_loader
        del test_loader
    ...
    print(f"\n✅ All splits completed. Final CSV saved to: {csv_path}")
    return all_results
